...
if ! is_true "$AUTO_ENABLE_EXTENSIONS"; then
  echo "[extensions] AUTO_ENABLE_EXTENSIONS=false -> skip"
  exit 0
fi
...
for db in $DB_LIST; do
  echo "[extensions] Ensuring extensions on db='${db}', schema='${AUTO_ENABLE_SCHEMA}' (postgis=${AUTO_ENABLE_POSTGIS}, vector=${AUTO_ENABLE_PGVECTOR})"
